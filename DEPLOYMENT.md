...
- в личке бот не может написать первым: каждый пользователь должен открыть бота и нажать Start или отправить любое сообщение, после этого клавиатура сможет прийти через `--announce` или по команде `/start`.
